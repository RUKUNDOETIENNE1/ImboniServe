...
async function loadWindow(start, end) {
    ...
    for (const e of entries) {
        ...
        if (!providerMap.has(key))
            providerMap.set(key, { gateway: e.gateway ?? null, success: 0, failed: 0 });
        const p = providerMap.get(key);
        if (e.eventType === 'PAYMENT_SUCCESS')
            p.success += 1;
...
function toTrend(a, b, c) {
    const t = (0, trend_utils_1.analyzeSeries)([a, b, c]);
    return { direction: t.direction, momentum: t.momentum, acceleration: t.acceleration };
}
function volatilityFromBuckets(buckets) {
...
async function revenueBuckets(start, end, segments) {
...
function weightedProviderReliability(providers) {
...
async function computeFinanceSnapshot() {
