...
exports.buildArbitrationSnapshot = buildArbitrationSnapshot;
...
function pickIssuesFromEvaluations() {
    const recs = (0, evaluation_engine_1.getEvaluatedRecords)();
...
function uniqueIssues() {
    const set = new Set([...pickIssuesFromEvaluations(), ...pickIssuesFromCEO()]);
...
function evaluationSupportFor(key) {
    const recs = (0, evaluation_engine_1.getEvaluatedRecords)().filter((r) => String(r.insightType) === key);
    if (recs.length === 0)
        return 0;
    const correct = recs.filter((r) => r.predictionCorrect).length;
    return correct / recs.length;
}
function ceoSupportFor(key) {
    const batches = (0, evaluation_engine_1.getCeoBatches)();
    const last = batches[0];
    if (!last)
        return 0;
...
function buildConsensusForKey(key, fin, auditPenalty) {
    const layers = {
    ...
    const consensus = confidence >= 0.6 ? 'AGREE' : confidence <= 0.3 ? 'DISAGREE' : 'UNCLEAR';
    return { key, consensus, confidence, layers };
}
function detectConflicts(fin, audit, accuracy) {
    const conflicts = [];
...
function computeDisagreementIndex(conflicts, accuracy, evaluatedTrustAvg) {
...
function deriveFinalState(consensusConfidence, disagreementIndex, truthScore) {
    if (consensusConfidence >= 0.75 && disagreementIndex < 30 && truthScore >= 80)
        return 'HEALTHY';
    if (consensusConfidence >= 0.6 && disagreementIndex < 50 && truthScore >= 70)
        return 'DEGRADED';
    if (disagreementIndex >= 50 || truthScore < 60)
        return 'UNSTABLE';
    return 'CONFLICTED';
}
async function buildArbitrationSnapshot() {
    ...
    const allRecords = (0, evaluation_engine_1.getRecords)();
    const accuracy = (0, prediction_validator_1.computeAccuracyMetrics)(evaluated, allRecords).overallAccuracy;
    ...
    const audit = await (0, truth_audit_engine_1.buildTruthAuditSnapshot)();
    // Finance (if enabled)
    ...
    if (process.env.DIE_INTELLIGENCE_KERNEL_SHADOW === 'true') {
        try {
            await Promise.all(issues.map((k) => (0, unified_intelligence_kernel_1.getEvidence)(k)));
        }
        catch { }
    }
    ...
    if (process.env.DIE_INTELLIGENCE_DUAL_READ_ENABLED === 'true') {
        try {
            const evs = await Promise.all(issues.map((k) => (0, unified_intelligence_kernel_1.getEvidence)(k)));
            const uikConsensus = evs.map((e, idx) => {
                const key = issues[idx];
                const conf = unified_intelligence_kernel_2.scoring.computeConsensusScore({ layerSupports: { finance: e.supports?.finance || 0, evaluation: e.supports?.evaluation || 0, temporal: e.supports?.temporal || 0, ceo: 0, reasoning: 0 }, auditPenalty: audit.contradictionReport.contradictionScore }).confidence;
            ...
            (0, convergence_engine_1.measureArbitrationConvergence)({
                legacyConsensus,
                uikConsensus,
