...
exports.businessReasoning = exports.BusinessReasoningEngine = void 0;
...
const TTL_MS = 60000;
let cache = null;
function countByCode(feed, code) {
    return feed.filter((i) => i.code === code).length;
}
function pickSamples(feed, codes, limit = 3) {
    const out = [];
...
class BusinessReasoningEngine {
    async generateInsights() {
        try {
            ...
            const governanceStates = governance_engine_service_1.governanceEngine.getAllStates();
            const recentAudit = governance_engine_service_1.governanceEngine.getRecentAuditEvents(100);
            const insights = [];
            ...
            if (process.env.DIE_INTELLIGENCE_RELIABILITY_ENABLED === 'true') {
                try {
                    const [{ buildBaselineProfile }, { compareToBaseline }, { calibrateConfidence }, { scoreStability }, { computeTrustScore }] = await Promise.all([
                        Promise.resolve().then(() => __importStar(require('../../die/intelligence-core/baseline-engine'))),
                        Promise.resolve().then(() => __importStar(require('../../die/intelligence-core/baseline-comparison'))),
                        Promise.resolve().then(() => __importStar(require('../../die/intelligence-core/confidence-calibration'))),
                        Promise.resolve().then(() => __importStar(require('../../die/intelligence-core/stability-engine'))),
                        Promise.resolve().then(() => __importStar(require('../../die/intelligence-core/trust-score'))),
                    ]);
                    ...
                    for (const ins of insights) {
                        ...
                        let baseDev = undefined;
                        if (metricKey && baseline.metrics[metricKey]) {
                            const hour = String(new Date().getHours());
                            const stat = baseline.metrics[metricKey].byHour[hour];
            ...
            if (process.env.DIE_INTELLIGENCE_EVALUATION_ENABLED === 'true') {
                try {
                    const { captureInsights } = await Promise.resolve().then(() => __importStar(require('../../die/evaluation/evaluation-engine')));
            ...
            console.debug('[BusinessReasoning] error (ignored):', e?.message);
            return [];
        }
    }
}
exports.BusinessReasoningEngine = BusinessReasoningEngine;
exports.businessReasoning = new BusinessReasoningEngine();
