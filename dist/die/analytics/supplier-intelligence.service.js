...
exports.SupplierIntelligenceService = void 0;
...
class SupplierIntelligenceService {
     ...
     */
    static async getSupplierMetrics(supplierId, businessId, dateRange = (0, analytics_utils_1.getDateRange)('year')) {
        ...
        const totalSpend = documents.reduce((sum, doc) => sum + (doc.totalCents || 0), 0);
        const monthlySpend = documents
            .filter((doc) => doc.createdAt >= monthAgo)
            .reduce((sum, doc) => sum + (doc.totalCents || 0), 0);
        const quarterlySpend = documents
            .filter((doc) => doc.createdAt >= quarterAgo)
            .reduce((sum, doc) => sum + (doc.totalCents || 0), 0);
        const yearlySpend = documents
            .filter((doc) => doc.createdAt >= yearAgo)
            .reduce((sum, doc) => sum + (doc.totalCents || 0), 0);
        const invoiceCount = documents.length;
        const averageInvoiceValue = (0, analytics_utils_1.safeDivide)(totalSpend, invoiceCount);
        const sortedDates = documents.map((d) => d.createdAt).sort((a, b) => a.getTime() - b.getTime());
        ...
        const reconciliationCount = documents.filter((d) => d.reconciliation).length;
        const successfulReconciliations = documents.filter((d) => d.reconciliation && ['MATCHED_PO', 'MATCHED_GRN'].includes(d.reconciliation.state)).length;
        const reconciliationSuccessRate = (0, analytics_utils_1.safeDivide)(successfulReconciliations, reconciliationCount);
        const totalAnomalies = documents.reduce((sum, doc) => sum + doc.anomalyAlerts.length, 0);
        const anomalyRate = (0, analytics_utils_1.safeDivide)(totalAnomalies, invoiceCount);
        const approvedCount = documents.filter((d) => ['APPROVED', 'APPLIED'].includes(d.status || d.lifecycleState || '')).length;
        const approvalRate = (0, analytics_utils_1.safeDivide)(approvedCount, invoiceCount);
        const reliabilityScore = (0, analytics_utils_1.calculateHealthScore)({
            successRate: reconciliationSuccessRate,
            anomalyRate: Math.min(anomalyRate, 1),
            approvalRate,
        });
        const riskScore = Math.round((1 - reconciliationSuccessRate) * 40 +
            Math.min(anomalyRate * 100, 40) +
            (daysSinceLastInvoice > 90 ? 20 : 0));
        const confidenceScore = Math.round(reconciliationSuccessRate * 50 +
     ...
     */
    static async getSupplierIntelligence(options) {
        ...
        const metricsPromises = supplierIds.map((supplierId) => this.getSupplierMetrics(supplierId, businessId, dateRange));
        ...
        const growthMetrics = await Promise.all(allMetrics.map(async (m) => {
            ...
            const last30Spend = last30Docs.reduce((sum, d) => sum + (d.totalCents || 0), 0);
            const prev30Spend = prev30Docs.reduce((sum, d) => sum + (d.totalCents || 0), 0);
            const growthRate = (0, analytics_utils_1.calculateGrowthRate)(last30Spend, prev30Spend);
            return { ...m, growthRate };
        }));
        ...
        const inactive = allMetrics
            ...
            .sort((a, b) => b.daysSinceLastInvoice - a.daysSinceLastInvoice)
            .slice(0, limit);
        ...
        const spendTrend = (0, analytics_utils_1.aggregateToTimeSeries)(allDocuments, (d) => d.createdAt, (d) => d.totalCents || 0, dateRange.from, dateRange.to, 'month');
        ...
        const averageSpend = (0, analytics_utils_1.safeDivide)(totalSpend, totalSuppliers);
...
exports.SupplierIntelligenceService = SupplierIntelligenceService;
