...
class ProductIntelligenceService {
     ...
     */
    static async getProductMetrics(productId, productType, businessId, dateRange = (0, analytics_utils_1.getDateRange)('year')) {
        ...
        if (productType === 'INVENTORY_ITEM') {
            ...
            if (!inventoryItem || inventoryItem.businessId !== businessId)
                return null;
        ...
        const purchaseVolume = items.reduce((sum, item) => sum + (item.quantity || 0), 0);
        const totalSpend = items.reduce((sum, item) => sum + (item.totalPriceCents || 0), 0);
        const prices = items
        ...
        const uniqueSuppliers = new Set(items.map((item) => item.scannedDocument.supplierId).filter((s) => s !== null));
        const supplierCount = uniqueSuppliers.size;
        const anomalyCount = items.reduce((sum, item) => sum + item.scannedDocument.anomalyAlerts.length, 0);
        const sortedDates = items
            .map((item) => item.scannedDocument.createdAt)
        ...
        const recent30Items = items.filter((item) => item.scannedDocument.createdAt >= days30Ago);
        const prev30Items = items.filter((item) => item.scannedDocument.createdAt < days30Ago &&
        ...
        const prev30Prices = prev30Items
            .map((item) => item.unitPriceCents)
            .filter((p) => p !== null && p > 0);
        const avg30 = recent30Prices.length > 0 ? recent30Prices.reduce((sum, p) => sum + p, 0) / recent30Prices.length : null;
        const avgPrev30 = prev30Prices.length > 0 ? prev30Prices.reduce((sum, p) => sum + p, 0) / prev30Prices.length : null;
        const priceChange30d = avg30 !== null && avgPrev30 !== null ? (0, analytics_utils_1.calculateGrowthRate)(avg30, avgPrev30) : null;
        const recent90Items = items.filter((item) => item.scannedDocument.createdAt >= days90Ago);
        const prev90Items = items.filter((item) => item.scannedDocument.createdAt < days90Ago &&
        ...
        const prev90Prices = prev90Items
            .map((item) => item.unitPriceCents)
            .filter((p) => p !== null && p > 0);
        const avg90 = recent90Prices.length > 0 ? recent90Prices.reduce((sum, p) => sum + p, 0) / recent90Prices.length : null;
        const avgPrev90 = prev90Prices.length > 0 ? prev90Prices.reduce((sum, p) => sum + p, 0) / prev90Prices.length : null;
        const priceChange90d = avg90 !== null && avgPrev90 !== null ? (0, analytics_utils_1.calculateGrowthRate)(avg90, avgPrev90) : null;
        return {
            productId,
            productName,
            productType,
     ...
     */
    static async getProductIntelligence(options) {
        ...
        const inventoryMetricsPromises = inventoryItems.map((item) => this.getProductMetrics(item.productId, 'INVENTORY_ITEM', businessId, dateRange));
        const supplierMetricsPromises = supplierProducts.map((item) => this.getProductMetrics(item.supplierProductId, 'SUPPLIER_PRODUCT', businessId, dateRange));
        const allMetrics = (await Promise.all([...inventoryMetricsPromises, ...supplierMetricsPromises])).filter((m) => m !== null);
        ...
        const fallingPrices = allMetrics
            ...
            .sort((a, b) => b.priceDecrease - a.priceDecrease)
            .slice(0, limit);
        // High risk (high anomaly rate or high volatility)
        const highRisk = allMetrics
            .filter((m) => {
            const anomalyRate = (0, analytics_utils_1.safeDivide)(m.anomalyCount, m.purchaseVolume);
            return anomalyRate > 0.2 || m.priceVolatility > 30;
        })
            .sort((a, b) => {
            const aRisk = (0, analytics_utils_1.safeDivide)(a.anomalyCount, a.purchaseVolume) + a.priceVolatility / 100;
            const bRisk = (0, analytics_utils_1.safeDivide)(b.anomalyCount, b.purchaseVolume) + b.priceVolatility / 100;
            return bRisk - aRisk;
        })
            .slice(0, limit);
        // Single supplier dependency
        const singleSupplierDeps = await Promise.all(allMetrics
            .filter((m) => m.supplierCount === 1 && m.totalSpend > 0)
            .slice(0, limit)
            .map(async (m) => {
            const item = await p.scannedDocumentItem.findFirst({
                where: m.productType === 'INVENTORY_ITEM'
        ...
        const opportunities = await Promise.all(multiSupplierProducts.slice(0, limit).map(async (m) => {
            const items = await p.scannedDocumentItem.findMany({
                where: m.productType === 'INVENTORY_ITEM'
                    ? { productId: m.productId }
                    : { supplierProductId: m.productId },
            ...
            const supplierPrices = items
                .filter((item) => item.unitPriceCents && item.scannedDocument.supplier)
                .map((item) => ({
            ...
            const sorted = averages.sort((a, b) => a.avgPrice - b.avgPrice);
            const lowest = sorted[0];
            const highest = sorted[sorted.length - 1];
            if (highest.avgPrice - lowest.avgPrice < lowest.avgPrice * 0.1)
                return null;
            return {
                productId: m.productId,
        ...
        const procurementOpportunities = opportunities.filter((o) => o !== null);
        const totalProducts = allMetrics.length;
        const activeProducts = allMetrics.filter((m) => m.lastPurchaseDate && (new Date().getTime() - m.lastPurchaseDate.getTime()) / (24 * 60 * 60 * 1000) <= 90).length;
        const totalSpend = allMetrics.reduce((sum, m) => sum + m.totalSpend, 0);
        const averagePrice = (0, analytics_utils_1.safeDivide)(allMetrics.reduce((sum, m) => sum + m.averagePrice, 0), totalProducts);
