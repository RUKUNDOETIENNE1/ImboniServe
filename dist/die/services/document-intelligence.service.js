...
exports.DocumentIntelligenceReplayService = void 0;
...
function computeConfidence(rows) {
    const vals = rows.map((r) => r.confidence).filter((v) => typeof v === 'number');
    if (vals.length === 0)
        return null;
    return Math.round((vals.reduce((a, b) => a + b, 0) / vals.length) * 1000) / 1000;
}
class DocumentIntelligenceReplayService {
    static async replayIntelligenceStage(scannedDocumentId) {
        const p = prisma_1.prisma;
        const headerRows = await p.extractedDocumentHeaderField.findMany({
            where: { scannedDocumentId },
            select: { fieldName: true, fieldValue: true, confidence: true },
        });
        const itemRows = await p.scannedDocumentItem.findMany({
            where: { scannedDocumentId },
        ...
        for (const item of itemRows) {
            const fields = await p.extractedDocumentLineField.findMany({
        ...
        await document_lifecycle_service_1.DocumentLifecycleService.transitionDocumentLifecycle(scannedDocumentId, document_lifecycle_service_1.DocumentLifecycleState.INTELLIGENCE_DONE, {
            confidenceOverall,
            validationScore,
            headerFieldsPromoted: Object.keys(headerUpdate).length,
            lineItemsEnriched,
        }, { expectedCurrentState: document_lifecycle_service_1.DocumentLifecycleState.EXTRACTED, stage: 'intelligence' });
        return {
            scannedDocumentId,
            headerFieldsPromoted: Object.keys(headerUpdate).length,
            lineItemsEnriched,
...
exports.DocumentIntelligenceReplayService = DocumentIntelligenceReplayService;
