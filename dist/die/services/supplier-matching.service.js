...
exports.SupplierMatchingService = void 0;
...
class SupplierMatchingService {
    ...
    static async findBestMatch(options) {
        const { businessId, rawSupplierName, autoMatchThreshold = this.DEFAULT_AUTO_MATCH_THRESHOLD, reviewSuggestionThreshold = this.DEFAULT_REVIEW_THRESHOLD, } = options;
        ...
        const scored = allSuppliers
            ...
            .filter((s) => s.score >= reviewSuggestionThreshold)
     ...
     */
    static async learnAlias(supplierId, rawName, matchConfidence) {
     ...
     */
    static async createSupplierLink(scannedDocumentId, supplierId, confidence, linkType) {
        const p = prisma_1.prisma;
     ...
     */
    static async resolveSupplier(scannedDocumentId, rawSupplierName, businessId, options) {
        const matchOptions = {
            businessId,
            rawSupplierName,
        ...
        if (match.supplierId && match.matchType !== 'NO_MATCH') {
            link = await this.createSupplierLink(scannedDocumentId, match.supplierId, match.confidence, match.matchType);
     ...
     */
    static async getSuggestions(rawSupplierName, businessId, limit = 5) {
        const normalizedInput = this.normalizeName(rawSupplierName);
        if (!normalizedInput)
            return [];
        const p = prisma_1.prisma;
     ...
     */
    static async bulkResolve(items, options) {
        const results = [];
        for (const item of items) {
            try {
                const result = await this.resolveSupplier(item.scannedDocumentId, item.rawSupplierName, item.businessId, options);
...
exports.SupplierMatchingService = SupplierMatchingService;
SupplierMatchingService.DEFAULT_AUTO_MATCH_THRESHOLD = 0.85;
SupplierMatchingService.DEFAULT_REVIEW_THRESHOLD = 0.60;
