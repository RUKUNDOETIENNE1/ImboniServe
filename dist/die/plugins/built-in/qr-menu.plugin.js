...
exports.QRMenuPlugin = void 0;
const qrcode_1 = __importDefault(require("qrcode"));
const nanoid_1 = require("nanoid");
const index_1 = require("../../../die/provider/index");
const plugin_events_1 = require("../core/plugin-events");
const cache_1 = require("../../../die/plugins/runtime/cache");
...
async function getBusinessSummary(prisma, businessId) {
    const cached = (0, cache_1.getCachedValue)(BUSINESS_CACHE_NAMESPACE, businessId);
    ...
    if (!business) {
        (0, cache_1.setCachedValue)(BUSINESS_CACHE_NAMESPACE, businessId, null, BUSINESS_CACHE_TTL_MS);
        return null;
    }
    const summary = {
        id: business.id,
    ...
    (0, cache_1.setCachedValue)(BUSINESS_CACHE_NAMESPACE, businessId, summary, BUSINESS_CACHE_TTL_MS);
    return summary;
}
function invalidateMenuCaches(businessId, menuId) {
    (0, cache_1.deleteCachedValue)(MENU_PAYLOAD_CACHE_NAMESPACE, menuId);
    (0, cache_1.deleteCachedValue)(DASHBOARD_METRICS_CACHE_NAMESPACE, businessId);
}
const providerChain = (0, index_1.buildProviderChain)();
let ensureTablePromise = null;
async function ensureQrMenuTable(prisma) {
    if (!ensureTablePromise) {
        ...
        "updatedAt" timestamptz NOT NULL DEFAULT NOW()
            ...
            const indexBusiness = 'CREATE INDEX IF NOT EXISTS plugin_data_qr_menu_business_idx ON "plugin_data_qr_menu" ("businessId");';
            const indexStatus = 'CREATE INDEX IF NOT EXISTS plugin_data_qr_menu_status_idx ON "plugin_data_qr_menu" ("status");';
...
function normalizeCategory(raw, fallback = 'General') {
    if (!raw)
        return fallback;
...
function parseMenuLines(textContent) {
...
async function runExtraction(buffer, mimeType) {
    for (const provider of providerChain) {
        try {
            if (!provider.supportsMime(mimeType))
                continue;
            ...
            if (Array.isArray(result?.lines) && result.lines.length > 0) {
                return result.lines
                    ...
                    if (Array.isArray(line.fields)) {
                        return line.fields.map((f) => f?.value ?? '').join(' ');
                    }
                    return '';
                })
                    .filter(Boolean)
            ...
            const fullText = result?.rawPayload?.fullText ??
                result?.rawPayload?.content ??
                result?.text;
...
function buildPublicUrl(menuId) {
    const base = process.env.NEXT_PUBLIC_APP_URL?.replace(/\/$/, '');
    const path = `/plugins/qr-menu/${menuId}`;
    return base ? `${base}${path}` : path;
}
async function ensureQrMenuRecord(prisma, data) {
    await ensureQrMenuTable(prisma);
    const prismaWithQr = prisma;
...
async function applyAiEnhancements(input) {
    // Placeholder for future AI enrichment pipeline (multi-language, pricing, branding)
    return {
        menuItems: input.menuItems,
    };
}
async function loadMenuRecord(prisma, menuId) {
    const prismaWithQr = prisma;
...
async function resolveMenuPayload(prisma, menuId, getPublicUrl, preloadedRecord) {
    const cached = (0, cache_1.getCachedValue)(MENU_PAYLOAD_CACHE_NAMESPACE, menuId);
    if (typeof cached !== 'undefined') {
        return cached;
    }
    const record = preloadedRecord ?? (await loadMenuRecord(prisma, menuId));
    if (!record) {
        (0, cache_1.setCachedValue)(MENU_PAYLOAD_CACHE_NAMESPACE, menuId, null, MENU_PAYLOAD_NEGATIVE_TTL_MS);
        return null;
    }
    ...
    (0, cache_1.setCachedValue)(MENU_PAYLOAD_CACHE_NAMESPACE, menuId, cacheValue, MENU_PAYLOAD_CACHE_TTL_MS);
    return cacheValue;
}
...
function toApiPayload(data, getPublicUrl) {
...
exports.QRMenuPlugin = {
    ...
    visibility: 'public',
    triggers: [
        plugin_events_1.DIE_PLUGIN_EVENTS.MENU_UPLOADED,
        plugin_events_1.DIE_PLUGIN_EVENTS.DOCUMENT_UPLOADED,
        plugin_events_1.DIE_PLUGIN_EVENTS.IMAGE_DETECTED,
    ],
    ...
    async execute(context) {
        ...
        const scanJobId = payload.scanJobId;
        ...
        const menuId = (0, nanoid_1.nanoid)(12);
        ...
        const [menuJson, qrCodeBuffer] = await Promise.all([
            ...
            qrcode_1.default.toBuffer(publicUrl, { type: 'png', margin: 1, width: 512 }),
        ]);
        const qrCodeUpload = await services.storage.saveBuffer(`${storagePrefix}/qr.png`, qrCodeBuffer, 'image/png');
    ...
    async render(context) {
        if (context.route.id === 'qr-menu.dashboard.overview') {
            ...
            const prismaWithQr = context.services.prisma;
            const cachedMetrics = (0, cache_1.getCachedValue)(DASHBOARD_METRICS_CACHE_NAMESPACE, businessId);
            ...
            const menusWithPayload = await Promise.all(records.map(async (record) => {
                ...
                const cachedPayload = await resolveMenuPayload(context.services.prisma, record.id, context.services.storage.getPublicUrl, preloaded);
            ...
            const menus = menusWithPayload.map(({ payload, itemCount }) => ({
                ...payload,
                itemCount,
            }));
            const totalItems = menusWithPayload.reduce((acc, entry) => acc + entry.itemCount, 0);
            const publishedMenus = menusWithPayload.reduce((acc, entry) => (entry.payload.publicUrl ? acc + 1 : acc), 0);
            ...
            if (!cachedMetrics) {
                (0, cache_1.setCachedValue)(DASHBOARD_METRICS_CACHE_NAMESPACE, businessId, metrics, DASHBOARD_METRICS_CACHE_TTL_MS);
                (async () => {
                    try {
                        const exact = await prismaWithQr.pluginQrMenu.count({ where: { businessId } });
                        (0, cache_1.setCachedValue)(DASHBOARD_METRICS_CACHE_NAMESPACE, businessId, { ...metrics, totalMenus: exact }, DASHBOARD_METRICS_CACHE_TTL_MS);
                    }
                    catch (err) {
