...
exports.marketplaceIntelligence = exports.MarketplaceIntelligenceService = void 0;
...
class MarketplaceIntelligenceService {
     ...
     */
    computeAdoptionMetrics(pluginId) {
        const states = governance_engine_service_1.governanceEngine.getAllStates().filter((s) => s.pluginId === pluginId);
        ...
        for (const s of states) {
            ...
            if (!byBusiness.has(key)) {
                ...
                if (toMillis(s.updatedAt) > toMillis(prev.updatedAt))
                    byBusiness.set(key, s);
            }
        }
        ...
        const adoptionScore = Math.round(Math.min(100, businessAdoptionCount * 10) * 0.5 + activationRate * 100 * 0.3 + installToEnableRatio * 100 * 0.2);
        return {
            adoptionScore,
     ...
     */
    computeUsageMetrics(pluginId, lookbackMinutes = 60) {
        const recent = governance_engine_service_1.governanceEngine.getRecentAuditEvents(200).filter((e) => e.pluginId === pluginId);
        let lastUsedAt = null;
        if (recent.length > 0) {
        ...
        const weight = (e) => (e.eventType === 'ENABLE' ? 3 : e.eventType === 'INSTALL' ? 2 : 1);
        const scoreFor = (events) => events.reduce((acc, e) => acc + weight(e), 0);
        const recentScore = scoreFor(recentWindow);
        const priorScore = scoreFor(priorWindow);
        let trendDirection = 'STABLE';
        if (recentScore > priorScore * 1.2)
            trendDirection = 'UP';
        else if (recentScore < priorScore * 0.8)
            trendDirection = 'DOWN';
     ...
     */
    computeStabilityMetrics(pluginId) {
        const states = governance_engine_service_1.governanceEngine.getAllStates().filter((s) => s.pluginId === pluginId);
        const recent = governance_engine_service_1.governanceEngine.getRecentAuditEvents(200).filter((e) => e.pluginId === pluginId);
        ...
        for (const s of states) {
            if (s.enableCount > s.installCount + 2)
                inconsistencyPenalty += 10;
            if (s.disableCount > s.enableCount + 2)
                inconsistencyPenalty += 10;
        }
     ...
     */
    computeForPlugin(pluginId) {
        const match = (0, registry_1.listMarketplacePlugins)().find((p) => p.id === pluginId);
        if (!match)
            return null;
        return {
            pluginId: match.id,
...
exports.MarketplaceIntelligenceService = MarketplaceIntelligenceService;
exports.marketplaceIntelligence = new MarketplaceIntelligenceService();
