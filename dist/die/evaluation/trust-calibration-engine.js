...
exports.computeTrustCalibration = computeTrustCalibration;
const BUCKET_DEFS = [
...
function computeTrustCalibration(evaluated) {
    const buckets = BUCKET_DEFS.map((d) => ({ ...d, evaluated: 0, correct: 0, correctRate: 0 }));
    ...
    for (const b of buckets)
        b.correctRate = b.evaluated ? b.correct / b.evaluated : 0;
    ...
    for (let i = 1; i < populated.length; i++) {
        if (populated[i].correctRate > populated[i - 1].correctRate + 0.05) {
            monotonic = false;
            break;
        }
    }
