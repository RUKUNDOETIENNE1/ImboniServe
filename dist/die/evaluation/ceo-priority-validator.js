...
exports.computeCeoPriorityAccuracy = computeCeoPriorityAccuracy;
function rankInsightSet(items) {
    const types = new Set();
    const domains = new Set();
    for (const i of items) {
        types.add(String(i.insightType));
        domains.add(i.domain);
    }
    return { types, domains };
}
function computeCeoPriorityAccuracy(batches) {
    ...
    for (const b of evaluated) {
        ...
        const a1 = actual[0];
        if (p1 && a1 && (p1.insightType === a1.insightType || p1.domain === a1.domain))
            top1 += 1;
