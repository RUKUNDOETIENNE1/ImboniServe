...
exports.computeDomainAccuracy = computeDomainAccuracy;
/**
 ...
 */
function computeAccuracyMetrics(evaluated, allRecords) {
...
function estimateFalseNegatives(evaluated) {
    ...
    for (const r of evaluated) {
        if (r.isPositivePrediction)
            continue;
        if (r.outcomeValue === undefined)
            continue;
        // For a growth prediction (UP) that turned into a decline => a missed problem.
        if (r.expectation === 'UP' && r.outcomeValue < r.baselineValue * 0.9)
            fn += 1;
    }
    return fn;
}
function computeDomainAccuracy(evaluated) {
    const map = new Map();
    for (const r of evaluated) {
