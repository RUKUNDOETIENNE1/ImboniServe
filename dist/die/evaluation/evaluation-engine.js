...
exports.captureInsights = captureInsights;
exports.evaluateDue = evaluateDue;
exports.getRecords = getRecords;
exports.getEvaluatedRecords = getEvaluatedRecords;
exports.getCeoBatches = getCeoBatches;
exports.getEvaluatedCeoBatches = getEvaluatedCeoBatches;
exports.__resetEvaluationStateForTests = __resetEvaluationStateForTests;
...
async function readMetric(key) {
    try {
        ...
        if (key === 'feed.campaignWeak') {
            const feed = (0, context_cache_1.getFeedHistoryWithin)(60 * 60 * 1000);
...
function judge(expectation, baseline, outcome) {
    const tol = 0.05;
    switch (expectation) {
        case 'UP':
 ...
 */
async function captureInsights(insights) {
    ...
    try {
        ...
        for (const k of metricKeys)
            metricValues.set(k, await readMetric(k));
        for (const i of insights) {
            const m = INSIGHT_MAP[i.type];
            if (!m)
                continue;
        ...
        const predictedOrder = ceo.map((c) => {
            ...
            return {
                ...
                baselineValue: metricValues.get(mm?.metricKey) ?? 0,
        ...
        if (predictedOrder.length > 0) {
            ...
            if (ceoBatches.length > CEO_BATCH_LIMIT)
                ceoBatches = ceoBatches.slice(0, CEO_BATCH_LIMIT);
        }
    }
    catch {
...
async function evaluateDue() {
    ...
    try {
        ...
        for (const r of dueRecords) {
            if (!metricCache.has(r.metricKey))
                metricCache.set(r.metricKey, await readMetric(r.metricKey));
        ...
        for (const b of dueBatches) {
            ...
            for (const p of b.predictedOrder) {
                if (!metricCache.has(p.metricKey))
                    metricCache.set(p.metricKey, await readMetric(p.metricKey));
        ...
        if (process.env.DIE_INTELLIGENCE_DUAL_READ_ENABLED === 'true') {
            try {
                ...
                const legacyAcc = (() => {
                    ...
                    const { computeAccuracyMetrics } = require('../../die/evaluation/prediction-validator');
                ...
                (0, convergence_engine_1.measureEvaluationConvergence)({ legacyAccuracy: legacyAcc, unsupported: true });
            }
            catch { }
        }
    }
...
function getCeoBatches() {
    return ceoBatches.slice();
}
function getEvaluatedCeoBatches() {
    return ceoBatches.filter((b) => !!b.evaluatedAt);
}
function __resetEvaluationStateForTests() {
    records = [];
    ceoBatches = [];
    idCounter = 0;
}
...
try {
    ...
    const uik = require('../../die/kernel/unified-intelligence-kernel');
    if (uik && typeof uik.registerUIKResolvers === 'function') {
        uik.registerUIKResolvers({
            getEvaluatedRecords: () => getEvaluatedRecords(),
            getRecords: () => getRecords(),
            getCeoBatches: () => getCeoBatches(),
        });
    }
}
catch { }
