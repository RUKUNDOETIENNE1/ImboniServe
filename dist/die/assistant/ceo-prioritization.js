...
exports.prioritizeForCEO = prioritizeForCEO;
function clamp01(n) { return Math.max(0, Math.min(1, n)); }
function weightForInsight(i) {
...
function prioritizeForCEO(insights) {
    const items = [];
    for (const i of insights) {
        const w = weightForInsight(i);
        const baseScore = (w.revenue * 0.4) + (w.operations * 0.3) + (w.customer * 0.2) + (w.urgency * 0.1);
        const trust = typeof i.trustScore === 'number' ? Math.max(0, Math.min(100, i.trustScore)) / 100 : 0.5;
        const score = Math.max(0, Math.min(1, baseScore * 0.6 + trust * 0.4));
        const impactType = w.revenue >= w.operations && w.revenue >= w.customer && w.revenue >= w.supply ? 'REVENUE'
