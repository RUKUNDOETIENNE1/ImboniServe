...
exports.controlPlane = exports.ControlPlaneService = void 0;
...
const registry_1 = require("../plugins/marketplace/registry");
class ControlPlaneService {
    ...
    async generateSnapshot() {
        ...
        const marketplacePlugins = (0, registry_1.listMarketplacePlugins)();
        // Count plugin states
        ...
        const pluginsWithMarketplaceMetadata = marketplacePlugins.filter((p) => p.category && p.pricingModel).length;
        ...
        const recentAuditEvents = governance_engine_service_1.governanceEngine.getRecentAuditEvents(100);
        ...
        const qrMenuGovernanceState = governance_engine_service_1.governanceEngine.getState('qr-menu');
        let qrMenuStatus = 'unknown';
        if (qrMenuPlugin) {
     ...
     */
    async getPluginEcosystemSummary() {
        const plugins = plugin_runner_1.pluginRunner.list();
        const marketplacePlugins = (0, registry_1.listMarketplacePlugins)();
        const summaries = [];
        for (const plugin of plugins) {
            const governanceState = governance_engine_service_1.governanceEngine.getState(plugin.id);
    ...
    async getSystemHealthReport() {
        ...
        const pluginRuntimeHealth = snapshot.totalPlugins > 0 ? 'healthy' : 'critical';
        ...
        const governanceLayerHealth = snapshot.governanceHealthScore >= 80
            ? 'healthy'
        ...
        const marketplaceLayerHealth = snapshot.marketplaceCoverage >= 80 ? 'healthy' : snapshot.marketplaceCoverage >= 50 ? 'degraded' : 'critical';
        ...
        const overallHealth = overallScore >= 80 ? 'healthy' : overallScore >= 50 ? 'degraded' : 'critical';
...
// Singleton instance
exports.controlPlane = new ControlPlaneService();
