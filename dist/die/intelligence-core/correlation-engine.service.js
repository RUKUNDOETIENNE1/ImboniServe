...
exports.correlationEngine = exports.CorrelationEngineService = void 0;
...
class CorrelationEngineService {
     ...
     */
    async detectInefficiencies() {
        const inefficiencies = [];
        const allStates = governance_engine_service_1.governanceEngine.getAllStates();
        const marketplacePlugins = (0, registry_1.listMarketplacePlugins)();
        const unusedPlugins = allStates.filter((s) => s.lifecycleState === 'INSTALLED' && s.enableCount === 0).length;
        if (unusedPlugins > 0) {
            inefficiencies.push({
    ...
    async detectRiskSignals() {
        ...
        try {
            ...
            const byCode = (code) => feed.filter((i) => i.code === code);
            const bySource = (pluginId) => feed.filter((i) => i.data?.sourceTag === pluginId);
            // Inventory Shortage Correlation
            const invShortage = feed.filter((i) => i.data?.sourceTag === 'inventory' && (i.code === 'INVENTORY_STOCK_LOW' || i.code === 'INVENTORY_STOCK_OUT' || i.code === 'INVENTORY_THRESHOLD_BREACH'));
            ...
            for (const low of invLow) {
                const itemId = low.data?.inventoryItemId;
                const lowTs = parseTs(low.timestamp);
                const restock = invRestocked.find((r) => r.data?.inventoryItemId === itemId && parseTs(r.timestamp) > lowTs);
                if (restock) {
            ...
            if (pairs.length > 0) {
                ...
                this._recoveryInsight = { avgMs, hours, samples: pairs.length };
            }
            // ================================
    ...
    async detectOptimizationCandidates() {
        ...
        const insight = this._recoveryInsight;
        if (insight) {
            candidates.push({
...
exports.CorrelationEngineService = CorrelationEngineService;
exports.correlationEngine = new CorrelationEngineService();
