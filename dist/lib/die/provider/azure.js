...
exports.AzureDocIntelligenceProvider = exports.AzureDIValidationError = exports.AzureDIEmptyResultError = exports.AzureDINetworkError = exports.AzureDITimeoutError = exports.AzureDIQuotaError = exports.AzureDIAuthError = exports.AzureDIConfigError = void 0;
...
// ---------------------------------------------------------------------------
function assertUsable(fields, lines) {
    const usableFields = (fields ?? []).filter((f) => f.value !== undefined && f.value !== null && String(f.value).trim() !== '');
    const usableLines = (lines ?? []).length;
    if (usableFields.length + usableLines < MIN_USABLE_FIELDS) {
        throw new AzureDIEmptyResultError(`Azure DI returned no usable content ` +
            `(fields=${usableFields.length}, lines=${usableLines}). ` +
            `Treating as extraction failure.`);
    }
}
function isUsableConfidence(confidence) {
...
// ---------------------------------------------------------------------------
class AzureDIConfigError extends Error {
    constructor(msg) {
        super(msg);
        this.name = 'AzureDIConfigError';
    }
}
exports.AzureDIConfigError = AzureDIConfigError;
...
exports.AzureDIAuthError = AzureDIAuthError;
class AzureDIQuotaError extends Error {
    constructor(msg) {
        super(msg);
        this.name = 'AzureDIQuotaError';
    }
}
exports.AzureDIQuotaError = AzureDIQuotaError;
...
exports.AzureDITimeoutError = AzureDITimeoutError;
...
exports.AzureDINetworkError = AzureDINetworkError;
...
exports.AzureDIEmptyResultError = AzureDIEmptyResultError;
class AzureDIValidationError extends Error {
    constructor(msg) {
        super(msg);
        this.name = 'AzureDIValidationError';
    }
}
exports.AzureDIValidationError = AzureDIValidationError;
// ---------------------------------------------------------------------------
// Main provider class
// ---------------------------------------------------------------------------
class AzureDocIntelligenceProvider {
    constructor() {
        ...
        if (!endpoint || !key) {
            throw new AzureDIConfigError('AZURE_DI_ENDPOINT and AZURE_DI_KEY must both be set to use Azure Document Intelligence');
        }
    ...
    async extract(input) {
        ...
        let analyzeUrl = `${this.endpoint}/documentintelligence/documentModels/${model}:analyze` +
        ...
        if (!operationUrl) {
            throw new AzureDIValidationError('Azure DI did not return Operation-Location header after document submit');
        }
    ...
    // ---------------------------------------------------------------------------
    async pollForResult(operationUrl, initialDelayMs) {
        ...
        if (firstDelay <= 0) {
            throw new AzureDITimeoutError(`Azure DI timeout budget exhausted before first poll (timeoutMs=${this.timeoutMs})`);
        }
        ...
        while (Date.now() < deadline) {
            ...
            if (remaining <= 0)
                break;
            const pollResp = await this.fetchWithTimeout(operationUrl, {
                method: 'GET',
        ...
        throw new AzureDITimeoutError(`Azure DI analysis did not complete within ${this.timeoutMs}ms`);
    }
    // ---------------------------------------------------------------------------
    // HTTP error handling: maps status codes to typed errors
    // ---------------------------------------------------------------------------
    async checkHttpError(response, stage) {
        if (response.ok)
            return;
        let body = '';
        try {
    ...
    // ---------------------------------------------------------------------------
    async fetchWithTimeout(url, init, overrideTimeoutMs) {
        ...
        try {
            ...
            if (err?.name === 'AbortError') {
                throw new AzureDITimeoutError(`Azure DI request timed out after ${ms}ms`);
            }
    ...
    // ---------------------------------------------------------------------------
    mapResult(body, model) {
        const analyzeResult = body?.analyzeResult;
        if (!analyzeResult || typeof analyzeResult !== 'object') {
            throw new AzureDIValidationError('Azure DI response missing analyzeResult object');
        }
        const fields = [];
        const lines = [];
        ...
        if (itemsField?.valueArray && Array.isArray(itemsField.valueArray)) {
            for (const item of itemsField.valueArray) {
                ...
                for (const [fieldName, fieldVal] of Object.entries(itemFields)) {
                    const fv = fieldVal;
                    const content = fv?.content ?? fv?.valueString ?? fv?.value ?? fv?.valueNumber ?? fv?.valueDate ?? '';
        ...
        if (fields.length === 0 && lines.length === 0) {
            const pageLines = analyzeResult.pages?.[0]?.lines ?? [];
