...
class ProductMatchingService {
    ...
    static normalizeName(name) {
        ...
        const muPat = 'kgs?|kilos?|kilograms?|grams?|gr|' +
    ...
    static calculateMatchScore(str1, str2) {
        ...
        const matchingWords = words1.filter(w1 => words2.some(w2 => w2 === w1 || w2.includes(w1) || w1.includes(w2))).length;
     ...
     */
    static async findInventoryItemMatch(businessId, rawProductName, autoMatchThreshold = this.DEFAULT_AUTO_MATCH_THRESHOLD, reviewSuggestionThreshold = this.DEFAULT_REVIEW_THRESHOLD) {
        ...
        const scored = allItems
            ...
            .filter((item) => item.score >= reviewSuggestionThreshold)
     ...
     */
    static async findSupplierProductMatch(supplierId, rawProductName, autoMatchThreshold = this.DEFAULT_AUTO_MATCH_THRESHOLD, reviewSuggestionThreshold = this.DEFAULT_REVIEW_THRESHOLD) {
        const normalizedInput = this.normalizeName(rawProductName);
        if (!normalizedInput) {
            return {
                productId: null,
     ...
     */
    static async findBestMatch(options) {
        const { businessId, supplierId, rawProductName, autoMatchThreshold = this.DEFAULT_AUTO_MATCH_THRESHOLD, reviewSuggestionThreshold = this.DEFAULT_REVIEW_THRESHOLD, } = options;
        // If we have a supplierId, try supplier products first
        let supplierResult = null;
        if (supplierId) {
            supplierResult = await this.findSupplierProductMatch(supplierId, rawProductName, autoMatchThreshold, reviewSuggestionThreshold);
        ...
        // Try inventory items
        const inventoryResult = await this.findInventoryItemMatch(businessId, rawProductName, autoMatchThreshold, reviewSuggestionThreshold);
        // If high confidence match, return immediately
     ...
     */
    static async learnAlias(rawName, options) {
        ...
        if (options.inventoryItemId)
            data.inventoryItemId = options.inventoryItemId;
        if (options.supplierProductId)
            data.supplierProductId = options.supplierProductId;
     ...
     */
    static async createProductLink(scannedDocumentId, productId, productType, confidence, linkType) {
        const p = prisma_1.prisma;
     ...
     */
    static async resolveProduct(scannedDocumentItemId, scannedDocumentId, rawProductName, businessId, supplierId, options) {
        const matchOptions = {
            businessId,
        ...
        if (match.productId && match.matchType !== 'NO_MATCH') {
            link = await this.createProductLink(scannedDocumentId, match.productId, match.productType, match.confidence, match.matchType);
     ...
     */
    static async resolveAllProducts(scannedDocumentId, businessId, supplierId, options) {
        const p = prisma_1.prisma;
        ...
        for (const item of items) {
            try {
                const result = await this.resolveProduct(item.id, scannedDocumentId, item.productName, businessId, supplierId, options);
                if (result.match.matchType === 'AUTO_MATCH')
                    matched++;
                else if (result.match.matchType === 'REVIEW_SUGGESTION')
                    suggestions++;
                else
                    unmatched++;
     ...
     */
    static async getSuggestions(rawProductName, businessId, supplierId, limit = 5) {
        const normalizedInput = this.normalizeName(rawProductName);
        if (!normalizedInput)
            return [];
        const p = prisma_1.prisma;
...
ProductMatchingService.DEFAULT_AUTO_MATCH_THRESHOLD = 0.85;
ProductMatchingService.DEFAULT_REVIEW_THRESHOLD = 0.60;
