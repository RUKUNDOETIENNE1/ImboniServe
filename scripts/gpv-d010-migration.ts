/**
 * GPV-D010: Data Migration for Pre-Existing Paid Orders
 *
 * Fixes orders that were paid BEFORE the GPV-D010 fix:
 * 1. Sets Sale.status = 'COMPLETED' for orders with paymentStatus='COMPLETED' and isPaid=true
 * 2. Updates PaymentTransaction.status = 'SUCCESS' and paidAt for linked transactions
 * 3. Creates missing FinancialLedgerEntry records with SALES domain
 *
 * This migration is idempotent — safe to run multiple times.
 */

import { PrismaClient, BillingEventType, LedgerDomain } from '@prisma/client'
import { logger } from '@/lib/logger'

const prisma = new PrismaClient()
const log = logger.child({ service: 'gpv-d010-migration' })
...
async function main() {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        if (sale.paymentTransactionId) {
          ...
          const existingLedger = await tx.financialLedgerEntry.findFirst({
            where: { businessId: sale.businessId, eventType: BillingEventType.PAYMENT_SUCCESS },
          })

          // Check if there's already a non-zero ledger entry for this business
  ...
  // Step 3: Clean up 0-amount PLATFORM ledger entries that were created by the old logBillingEvent bug
  console.log('\n--- Step 3: Clean up 0-amount PLATFORM ledger entries ---')
  const zeroAmountEntries = await prisma.financialLedgerEntry.deleteMany({
    where: {
