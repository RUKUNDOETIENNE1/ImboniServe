...
export default function MarketingAttentionCenter({ items, loading, onNavigate }: Props) {
  if (loading) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-6 animate-pulse">
        <div className="h-5 w-48 bg-slate-200 rounded mb-4" />
        <div className="space-y-2">
          {[...Array(3)].map((_, i) => <div key={i} className="h-16 w-full bg-slate-100 rounded" />)}
        </div>
      </div>
    )
  }
  ...
  if (!items || items.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        <h3 className="text-base font-bold text-slate-900 mb-2">Marketing Attention Center</h3>
        <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-50 border border-emerald-200">
          <AlertOctagon className="w-4 h-4 text-emerald-500" />
          <p className="text-sm text-emerald-900">No marketing items require attention. Growth engine is running smoothly.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6">
      <h3 className="text-base font-bold text-slate-900 mb-4">Marketing Attention Center</h3>
