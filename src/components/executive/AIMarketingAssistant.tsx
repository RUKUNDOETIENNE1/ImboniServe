...
export default function AIMarketingAssistant({ recommendations, loading, onNavigate }: Props) {
  if (loading) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-6 animate-pulse">
        <div className="h-5 w-40 bg-slate-200 rounded mb-4" />
        <div className="space-y-3">
          {[...Array(2)].map((_, i) => <div key={i} className="h-32 w-full bg-slate-100 rounded" />)}
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-base font-bold text-slate-900">AI Marketing Assistant</h3>
        </div>
        <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-50 border border-emerald-200">
          <CheckCircle className="w-4 h-4 text-emerald-500" />
          <p className="text-sm text-emerald-900">No marketing issues detected. Growth engine is performing well.</p>
        </div>
      </div>
    )
  }
            ...
            <div key={i} className="rounded-xl border border-slate-200 p-4 bg-slate-50">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-medium text-slate-700">{rec.expectedImpact}</p>
                </div>
              </div>
