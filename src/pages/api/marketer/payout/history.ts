...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Get payouts
    const payouts = await MarketerPayoutService.getPayoutsForMarketer(marketer.id, {
      status: status as any,
      limit: parseInt(limit as string),
      offset: parseInt(offset as string)
    });

    logger.info('Marketer payout history accessed', {
    ...
    logger.error('Payout history fetch failed', { error });
