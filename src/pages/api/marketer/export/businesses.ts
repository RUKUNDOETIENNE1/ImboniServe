...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const csv = await RevenueExportService.exportBusinessesCSV(marketer.id);

    logger.info('Businesses CSV exported', { marketerId: marketer.id });

    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename="businesses-${marketer.referralCode}-${new Date().toISOString().split('T')[0]}.csv"`);
