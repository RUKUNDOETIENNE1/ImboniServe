...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    logger.info('Commissions CSV exported', { marketerId: marketer.id });

    // Set headers for CSV download
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename="commissions-${marketer.referralCode}-${new Date().toISOString().split('T')[0]}.csv"`);
