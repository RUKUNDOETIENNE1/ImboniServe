...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const payment = await prisma.paymentTransaction.findFirst({
    where: { id: id as string, businessId, marketplaceOrderId: { not: null } },
    include: {
      marketplaceOrder: { include: { items: { include: { product: true } } } },
      business: true,
    },
  })
  ...
  const order = (payment as any).marketplaceOrder
  ...
  })) || [{ description: 'Marketplace Order', total: payment.amountCents }]
  ...
  res.setHeader('Content-Disposition', `inline; filename="receipt-${payment.invoiceNumber}.html"`)
