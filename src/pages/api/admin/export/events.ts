...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { startDate, endDate, type, entityType } = req.query;

    const csv = await RevenueExportService.exportEventsCSV({
    ...
    logger.info('Events CSV exported by admin', { adminId: session.user?.id });

    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename="revenue-events-${new Date().toISOString().split('T')[0]}.csv"`);
